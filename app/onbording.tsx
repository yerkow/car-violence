...
export default function Onbording() {
    ...
    return <View style={[style.container]}>
        ...
        {step !== 4 && <Stepper current={step} handlePress={(stepIdx: number) => {
...
const Stepper = ({ current, handlePress }: { current: number, handlePress: (stepIdx: number) => void }) => {
    const [dim, setDim] = useState({ width: 100, height: 20 })
    const stepWidth = dim.width / 4
    ...
    useEffect(() => {
        stepPositionX.value = withSpring(stepWidth * current, { duration: 1500 })
    }, [current])
